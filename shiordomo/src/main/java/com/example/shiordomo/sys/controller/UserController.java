...
import com.example.shiordomo.sys.common.PinyinUtils;
...
import com.example.shiordomo.sys.vo.UserVo;
...
//        queryWrapper.eq(StringUtils.isNotBlank(userVo.getAddress()), "address", userVo.getAddress());
        queryWrapper.eq("type", Constast.USER_TYPE_NORMAL);//查询系统用户,超级管理员无法查询
        ...
        for (User user : list) {
            ...
            if(deptid!=null) {
                Dept one = deptService.getById(deptid); //一定要传id，不能getOne()
